...
def seed_db():
    ...
    plan1 = SalesPlan(
        name="Q1 2025 Sales Campaign",
        description="First quarter sales targets focusing on new client acquisition",
    ...
    for i, name in enumerate(seller_names[:3], start=100):
        ...
        print(f"Added seller: {name} (ID: {i}) to plan: {plan1.name}")
    ...
    plan2 = SalesPlan(
        name="Q2 2025 Expansion Plan",
        description="Second quarter targets focusing on expanding existing accounts",
        target_amount=75000.0,
        start_date=next_quarter_start.strftime('%Y-%m-%d'),
        end_date=next_quarter_end.strftime('%Y-%m-%d')
    )
    ...
    for i, name in enumerate(seller_names, start=200):
        ...
        db.session.add(seller)
        print(f"Added seller: {name} (ID: {i}) to plan: {plan2.name}")
    ...
    plan3 = SalesPlan(
        name="2025 Annual Targets",
        description="Overall annual sales targets for all territories",
    ...
    for i, name in enumerate(seller_names[3:], start=300):
        ...
        db.session.add(seller)
        print(f"Added seller: {name} (ID: {i}) to plan: {plan3.name}")
