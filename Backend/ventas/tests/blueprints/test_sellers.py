...
class TestSellers:
    def test_create_seller(self, client, db_session):
        ...
        # Check that the seller was created in the database
        seller = Seller.query.filter_by(seller_id=100).first()
        assert seller is not None
        assert seller.name == "Test Seller"
    
    def test_get_sellers(self, client, db_session):
        ...
        seller_ids = [seller['seller_id'] for seller in response.json]
        assert 101 in seller_ids
        assert 102 in seller_ids
    
    def test_get_seller_by_id(self, client, db_session):
    ...
    def test_update_seller(self, client, db_session):
        ...
        updated_seller = Seller.query.filter_by(seller_id=300).first()
        assert updated_seller.name == "Updated Name"
    
    def test_delete_seller(self, client, db_session):
        ...
        deleted_seller = Seller.query.filter_by(seller_id=400).first()
